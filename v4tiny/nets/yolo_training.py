...
class YOLOLoss(nn.Module):
    ...
    def box_eiou(self, b1, b2):
        ...
        返回为：
        -------
        eiou: tensor, shape=(batch, feat_w, feat_h, anchor_num, 1)
        """
        # ----------------------------------------------------#
        #   求出预测框左上角右下角
        # ----------------------------------------------------#
        ...
        #   计算 EIoU 损失
        # ----------------------------------------------------#
        eiou = iou - (center_distance / torch.clamp(enclose_diagonal, min=1e-6)) * (1 - iou)
        ...
        eiou = eiou - alpha * v  # 这里是 EIoU 损失的核心计算

        # ----------------------------------------------------#
        #   可选的额外项（如果有的话）
        # ----------------------------------------------------#
        # 你可以根据需要在这里添加更多额外的几何项或其他损失项，例如调整 alpha 和 beta
        # 例如，额外的几何因素或者其他参数的优化
